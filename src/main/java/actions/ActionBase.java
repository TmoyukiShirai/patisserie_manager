...
public abstract class ActionBase {
     ...
     * @throws IOException
     */
    protected void redirect(ForwardConst action, ForwardConst command, String pram)
            throws ServletException, IOException {
     ...
     * @return 変換後数値
     */
    protected double toDoubleNumber(String strNumber) {
        double number = 0;
        try {
    ...
    /**
     * リクエストスコープから指定されたパラメータの値を複数取得し、返却する
     * @param key パラメータ名
     * @return パラメータの値
     */
  ...
  /** Integer
   * リクエストスコープから指定されたパラメータの値を取得し、Integer型で返却する
   * @param key パラメータ名
   * @return パラメータの値
   */
  protected Integer getIntegerRequestParam(AttributeConst key) {
      String tmp = request.getParameter(key.getValue());
      if (tmp == null || "".equals(tmp)) {  //空文字対策
          return null;
      }
      return Integer.parseInt(request.getParameter(key.getValue()));
  }

//    ダブル
    /**
     * リクエストスコープから指定されたパラメータの値を取得し、ダブル型で返却する
     * @param key パラメータ名
     * @return パラメータの値
     */
    protected Double getDoubleRequestParam(AttributeConst key) {
        String tmp = request.getParameter(key.getValue());
        if (tmp == null || "".equals(tmp)) {  //空文字対策
            return null;
        }
        return Double.parseDouble(request.getParameter(key.getValue()));
    }

//  LocalDate
  /**
   * リクエストスコープから指定されたパラメータの値を取得し、LocalDate型で返却する
   * @param key パラメータ名
   * @return パラメータの値
   */
  protected LocalDate getLocalDateRequestParam(AttributeConst key) {
      return LocalDate.parse(request.getParameter(key.getValue()));
  }
}