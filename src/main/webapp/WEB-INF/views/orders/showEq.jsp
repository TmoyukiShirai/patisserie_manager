    ...
    <c:param name="content">
        ...
        <form method="POST" action="<c:url value='?action=${actOrder}&command=${commCrt}' />">
            <h3>
                ...
                <input type="hidden" value="${saler}" name="${AttributeConst.EQ_SALER.getValue() }"/>
                    ...
                    <tbody>
                        ...
                        <c:forEach var="equipment" items="${equipments}" varStatus="status">
                         <tr id="order_list">
                            ...
                            <td class="product"><c:out value="${equipment.name}" /><input type="hidden" value="${equipment.id}" name="${AttributeConst.EQ_ID.getValue() }"/></td>
                            <td class="maker"><c:out value="${equipment.maker}" /><input type="hidden" value="${equipment.price}" name="${AttributeConst.EQ_PRICE.getValue() }"/></td>
                            <td class="amount"><c:out value="${equipment.amount}${equipment.amount_u}" /></td>
            ...
            <div id="pagination">
                ...
                <c:forEach var="i" begin="1" end="${((equipments_count - 1) / maxRow) + 1}" step="1">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a href="<c:url value='?action=${actEq}&command=${commIdx}&page=${i}' />"><c:out value="${i}" /></a>&nbsp;
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
            </div>
